...
for fold , (train_inds, test_inds) in enumerate(cv.split(X=train_df, y=train_df.label, groups=train_df.img_name)):
    ...
    train_df_cv.to_csv(savePath+f'nodule_train_fold{fold}_finalgen.csv')
    f=open(f"{savePath}nodule_train_fold{fold}_finalgen.txt", "a+")
    for idx, row in train_df_cv.iterrows():
        name = row.Path
        f.write(f"{name}\n")
    f.close()
    
    val_df_cv.to_csv(savePath+f'nodule_val_fold{fold}_finalgen.csv')
    f=open(f"{savePath}nodule_val_fold{fold}_finalgen.txt", "a+")
